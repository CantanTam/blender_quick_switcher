...
def draw_add_to_switcher_greasepencilweight(self, context):
    ...
    if op and op.bl_rna.identifier == "GREASE_PENCIL_OT_vertex_group_normalize_all":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"全部规格化\"⟶Switcher", icon='PLUS').action = 'action.greasepencilweight_vertex_group_normalize_all'

    elif op and op.bl_rna.identifier == "GREASE_PENCIL_OT_vertex_group_normalize":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"规格化\"⟶Switcher", icon='PLUS').action = 'action.greasepencilweight_vertex_group_normalize'

    elif op and op.bl_rna.identifier == "GREASE_PENCIL_OT_weight_invert":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"反转\"⟶Switcher", icon='PLUS').action = 'action.greasepencilweight_vertex_group_invert'

    elif op and op.bl_rna.identifier == "GREASE_PENCIL_OT_vertex_group_smooth":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"光滑\"⟶Switcher", icon='PLUS').action = 'action.greasepencilweight_vertex_group_smooth'

    elif op and op.bl_rna.identifier == "GREASE_PENCIL_OT_weight_sample":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"采样权重\"⟶Switcher", icon='PLUS').action = 'action.greasepencilweight_weight_sample'

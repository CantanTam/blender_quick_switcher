...
# ，无法在函数内部添加参数，而 bpy.ops.popup.quick_menu_one() 是无法调出自定义菜单的
# ，所以需要再构建一个函数，调用 bpy.ops.popup.quick_menu_one('INVOKE_DEFAULT')
...
class CALLOUT_QUICK_MENU_OT_two(bpy.types.Operator):
    ...
    def execute(self, context):
        bpy.ops.popup.quick_menu_two('INVOKE_DEFAULT')
        return {'FINISHED'}

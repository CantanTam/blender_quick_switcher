...
class QUICK_POPUP_MENU_OT_one_meshedit(bpy.types.Menu):
    ...
    def poll(cls, context):
        return context.preferences.addons[cls.ADDON_NAME].preferences.panelone_mode2_visible
    ...
    def draw(self, context):
        ...
        if prefs.panelone_mode2_col1_title.strip() != "" :
            col1 = row.column()
        if prefs.panelone_mode2_col2_title.strip() != "":
            col2 = row.column()
        if prefs.panelone_mode2_col3_title.strip() != "":
            col3 = row.column()
        if prefs.panelone_mode2_col4_title.strip() != "":
            col4 = row.column()
        if prefs.panelone_mode2_col5_title.strip() != "":
            col5 = row.column()
        if prefs.panelone_mode2_col6_title.strip() != "":
            col6 = row.column()
        if prefs.panelone_mode2_col7_title.strip() != "":
            col7 = row.column()
        if prefs.panelone_mode2_col8_title.strip() != "":
            col8 = row.column()

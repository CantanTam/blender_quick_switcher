...
class QUICK_POPUP_MENU_OT_one_greasepenciledit(bpy.types.Menu):
    bl_idname = "popup.quick_menu_one_greasepenciledit"
    bl_label = "蜡笔编辑模式"
    ...
    def poll(cls, context):
        return context.preferences.addons[cls.ADDON_NAME].preferences.panelone_mode12_visible
    ...
    def draw(self, context):
        ...
        if prefs.panelone_mode12_col1_title.strip() != "" :
            col1 = row.column()
        if prefs.panelone_mode12_col2_title.strip() != "":
            col2 = row.column()
        if prefs.panelone_mode12_col3_title.strip() != "":
            col3 = row.column()
        if prefs.panelone_mode12_col4_title.strip() != "":
            col4 = row.column()
        if prefs.panelone_mode12_col5_title.strip() != "":
            col5 = row.column()
        if prefs.panelone_mode12_col6_title.strip() != "":
            col6 = row.column()
        if prefs.panelone_mode12_col7_title.strip() != "":
            col7 = row.column()
        if prefs.panelone_mode12_col8_title.strip() != "":
            col8 = row.column()

        # 第一列菜单项
        if prefs.panelone_mode12_col1_title.strip() != "":
            col1.label(text=prefs.panelone_mode12_col1_title, icon='PRESET')
            col1.separator()
            for i in range(1, 11):
                col1_button_value = getattr(prefs, f"panelone_mode12_col1_button{i}")
        ...
        # 第二列菜单项
        if prefs.panelone_mode12_col2_title.strip() != "":
            col2.label(text=prefs.panelone_mode12_col2_title, icon='PRESET')
            col2.separator()
            for i in range(1, 11):
                col2_button_value = getattr(prefs, f"panelone_mode12_col2_button{i}")
        ...
        # 第三列菜单项
        if prefs.panelone_mode12_col3_title.strip() != "":
            col3.label(text=prefs.panelone_mode12_col3_title, icon='PRESET')
            col3.separator()
            for i in range(1, 11):
                col3_button_value = getattr(prefs, f"panelone_mode12_col3_button{i}")
        ...
        # 第四列菜单项
        if prefs.panelone_mode12_col4_title.strip() != "":
            col4.label(text=prefs.panelone_mode12_col4_title, icon='PRESET')
            col4.separator()
            for i in range(1, 11):
                col4_button_value = getattr(prefs, f"panelone_mode12_col4_button{i}")
        ...
        # 第五列菜单项
        if prefs.panelone_mode12_col5_title.strip() != "":
            col5.label(text=prefs.panelone_mode12_col5_title, icon='PRESET')
            col5.separator()
            for i in range(1, 11):
                col5_button_value = getattr(prefs, f"panelone_mode12_col5_button{i}")
        ...
        # 第六列菜单项
        if prefs.panelone_mode12_col6_title.strip() != "":
            col6.label(text=prefs.panelone_mode12_col6_title, icon='PRESET')
            col6.separator()
            for i in range(1, 11):
                col6_button_value = getattr(prefs, f"panelone_mode12_col6_button{i}")
        ...
        # 第七列菜单项
        if prefs.panelone_mode12_col7_title.strip() != "":
            col7.label(text=prefs.panelone_mode12_col7_title, icon='PRESET')
            col7.separator()
            for i in range(1, 11):
                col7_button_value = getattr(prefs, f"panelone_mode12_col7_button{i}")
        ...
        # 第八列菜单项
        if prefs.panelone_mode12_col8_title.strip() != "":
            col8.label(text=prefs.panelone_mode12_col8_title, icon='PRESET')
            col8.separator()
            for i in range(1, 11):
                col8_button_value = getattr(prefs, f"panelone_mode12_col8_button{i}")

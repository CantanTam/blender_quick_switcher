...
def draw_add_to_switcher_global_view(self, context):
    ...
    if op and op.bl_rna.identifier == "VIEW3D_OT_view_selected":
        ...
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"局部视图\"添加到Switcher", icon='RADIOBUT_OFF').action = 'button.action_view3d_localview'
        layout.operator("call.add_to_switcher_menu", text="添加分隔符到Switcher", icon='REMOVE').action = 'SEPARATOR'
        
    elif op and op.bl_rna.identifier == "VIEW3D_OT_localview_remove_from":
        layout = self.layout
        layout.separator()
        layout.operator("call.add_to_switcher_menu", text="\"从局部视图中移除\"添加到Switcher", icon='RADIOBUT_OFF').action = 'view3d.localview_remove_from'
        layout.operator("call.add_to_switcher_menu", text="添加分隔符到Switcher", icon='REMOVE').action = 'SEPARATOR'
